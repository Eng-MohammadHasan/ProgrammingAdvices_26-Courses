...
unsigned short CalculateDateDifferenceInDays(const sDate& Date1, const sDate& Date2 , bool IncludeEndDay = false)
{
	...
	{
		TotalDays = NumberOfDaysFromTheBeginningOfTheYear(Date2) - NumberOfDaysFromTheBeginningOfTheYear(Date1);
	}
	else
	{
		// Remaining days in Date1's year
		TotalDays =
			NumberOfDaysInAYear(Date1.Year) -
			NumberOfDaysFromTheBeginningOfTheYear(Date1);

		// Full years between Date1 and Date2
		for (unsigned short Year = Date1.Year + 1;
			Year < Date2.Year;
			Year++)
		{
		...
		// Days elapsed in Date2's year
		TotalDays +=
			NumberOfDaysFromTheBeginningOfTheYear(Date2);
	}
	
	return (IncludeEndDay) ? TotalDays + 1 : TotalDays;
}
