...
sDate GetDateFromDayOfYear(unsigned short DayOfYear, unsigned short Year)
{
	sDate Date;
	unsigned short RemainingDays = DayOfYear;
...
sDate GetDateAfterAddingDays(int DaysToAdd , const sDate& Date)
{
	sDate NewDate = Date;

	unsigned short DayOrderInYear = NumberOfDaysFromTheBeginningOfTheYear(Date.Day, Date.Month, Date.Year);
	unsigned short CurrentYearDays = NumberOfDaysInAYear(NewDate.Year);
	int RemainingDays = DaysToAdd;
	
	RemainingDays += DayOrderInYear;

	while (RemainingDays > CurrentYearDays)
	{
		RemainingDays -= CurrentYearDays;
		NewDate.Year++;
		CurrentYearDays = NumberOfDaysInAYear(NewDate.Year);
	}
	
	NewDate = GetDateFromDayOfYear(RemainingDays, NewDate.Year);
